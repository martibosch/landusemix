...
def getPolygons_LU_Region(df_polygon, polygon_shapes, df_polygon_sq, polygon_shapes_sq):
	...
	"""
	toPolygons_nonResiAct_LU_region = [ ( Polygon(polygon_shapes[i].points).buffer(0) , "other" ) for i in df_polygon.loc[ ( df_polygon['landuse'].isin(classif_uses.landuse_notResidentialActivity) ) & (df_polygon['building'].isin(['']) )].index  ]
	toPolygons_activities_LU_region = [ ( Polygon(polygon_shapes[i].points).buffer(0) , df_polygon.loc[i,'landuse'] ) for i in df_polygon.loc[ ( df_polygon['landuse'].isin(classif_uses.landuse_activities) ) & (df_polygon['building'].isin(['']) )].index  ]
	toPolygons_residential_LU_region = [ ( Polygon(polygon_shapes[i].points).buffer(0) , "residential" ) for i in df_polygon.loc[ ( df_polygon['landuse'].isin(classif_uses.landuse_residential) ) & (df_polygon['building'].isin(['']) )].index  ]
	
	sq_toPolygons_nonResiAct_LU_region = [ ( Polygon(polygon_shapes_sq[i].points).buffer(0) , "other" ) for i in df_polygon_sq.loc[ ( df_polygon_sq['landuse'].isin(classif_uses.landuse_notResidentialActivity) ) & (df_polygon_sq['building'].isin(['']) )].index ]
	sq_toPolygons_residential_LU_region = [ ( Polygon(polygon_shapes_sq[i].points).buffer(0) , "residential" ) for i in df_polygon_sq.loc[ ( df_polygon_sq['landuse'].isin(classif_uses.landuse_residential) ) & (df_polygon_sq['building'].isin(['']) )].index ]
	sq_toPolygons_activities_LU_region = [ ( Polygon(polygon_shapes_sq[i].points).buffer(0) , df_polygon_sq.loc[i,'landuse'] ) for i in df_polygon_sq.loc[ ( df_polygon_sq['landuse'].isin(classif_uses.landuse_activities) ) & (df_polygon_sq['building'].isin(['']) )].index ]
	...
	def getKey(item): # Sort by area
		return item[0].area
	np_Polygons_LU_Region = sorted(np_Polygons_LU_Region, key=getKey)
	# Convert to DataFrame to iterate rows, containing Polygon and Classification as columns
	df_Polygons_LU_Region = pd.DataFrame(np_Polygons_LU_Region, columns=['Polygon', 'Classification'])

	return df_Polygons_LU_Region#np_Polygons_LU_Region
...
def inferUseGivenContainingPolygons(idx_ToInfer_poly, full_containingPolygons):
	...
	for idx, containingPoly in zip(idx_ToInfer_poly, full_containingPolygons):
...
def idx_polygonsToInfer(df_polygon):
...
def filterInferredColumns(sub_selection):
	...
	"""
	filteredColumns_Selection = sub_selection[['osm_id']].copy() # Copy to avoid setting warning
...
def testValid(polygon_shapes):
	""" Check if all polygons are valid
	Due to rounding, some polygons might be invalid during their construction: Check if this is the case
	buffer(0) solves the problem
	"""
...
def main(polygon_shapefile, poly_shp_several_quadrants, suffix_out_shp):
	...
	# Get the polygons which need to be inferred: building=yes, afterwards Null values
	idx_polys_ToInfer = idx_polygonsToInfer(df_polygon) #ToInfer_poly
	polygons_ToInfer = [ Polygon(polygon_shapes[i].points).buffer(0) for i in idx_polys_ToInfer ] #toPolygons_ToInfer_poly

	###

	# Get the polygons and its classification with a valid land use
	#np_Polygons_LU_Region = getPolygons_LU_Region(df_polygon, polygon_shapes, df_polygon_sq, polygon_shapes_sq)
	df_Polygons_LU_Region = getPolygons_LU_Region(df_polygon, polygon_shapes, df_polygon_sq, polygon_shapes_sq)

	################
	
	if (parameters.USE_verbose):
		print("infer_poly_uses. Polygons with Landuse Region:", len(df_Polygons_LU_Region), "Polygons to infer", len(idx_polys_ToInfer))

	################

	# Calculate for each polygon to infer, the smallest containing polygon which has a defined landuse
	full_containingPolygons = calculateContainingPolygons(df_Polygons_LU_Region, polygons_ToInfer)

	# Get the inferred uses
	inferred_poly_activity, inferred_poly_residential , inferred_poly_uncertain = inferUseGivenContainingPolygons(idx_polys_ToInfer, full_containingPolygons)
