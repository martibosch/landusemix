...
def clip_PolygonFile(in_shp, out_shp):
	...
	if (parameters.USE_verbose):
		print("Clip polygon file:",in_shp)
	...
	for poly in r.iterShapeRecords():
	    ...
	    try: # Try to get the points
	    	Poly_ = Polygon(geom.points)
	    except IndexError: # If error, geometry is empty, then ignore
	    	#print('Index error at retrieving geometry points. Ignoring geometry')
	    	continue

	    quadrant = getTouchingQuadrants(PolygonQuadrants, Poly_)
