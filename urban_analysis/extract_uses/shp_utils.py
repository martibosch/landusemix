...
################################################
### Read data-set shapefile

# Read dataset: Shapes and attributes
def read_shp_dbf(file_shape, decode_bytes = True):
	...
	pd_dataframe = pd.DataFrame(reader_shp.records(), columns = columns)
	# Might need to decode
	if (decode_bytes): pd_dataframe = pd_dataframe.applymap(ByteToStr)
...
def toFile(fileName, shapes, shape_attrs, shapefileType, fields = None):
	...
	if (not(os.path.isfile(fileName+'.prj'))):
		try:
			fn_prefix = '/'.join(i for i in fileName.split('/')[:-1]) + '/'
			shutil.copy(fn_prefix+'sample.prj',fileName+'.prj')
