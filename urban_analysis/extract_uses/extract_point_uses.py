...
def main(points_shapefile, suffix_out_shp):
	...
	sub_activities_amenities = df_point.loc[df_point['amenity'].isin(classif_uses.amenities_activities)]
	sub_activities_leisure = df_point.loc[df_point['leisure'].isin(classif_uses.leisure_activies)]
	sub_activities_buildings = df_point.loc[df_point['building'].isin(classif_uses.building_activities)]
	sub_activities_landuse = df_point.loc[df_point['landuse'].isin(classif_uses.landuse_activities)]
	### Residential: Points
	sub_residential_buildings = df_point.loc[df_point['building'].isin(classif_uses.building_residential)]
	sub_residential_landuse = df_point.loc[df_point['landuse'].isin(classif_uses.landuse_residential)]
	...
	activities_pts = pd.concat([pts_activities_shops,pts_activities_amenities,pts_activities_leisure,pts_activities_buildings,pts_activities_landuse])
	residential_pts = pd.concat([pts_residential_buildings,pts_residential_landuse])
	...
	if (parameters.USE_dropDuplicates):
		...
		residential_pts = residential_pts.groupby(residential_pts.index).first()
	####################################################################################
	### Save to file
	utils.toFile(parameters.fn_prefix+parameters.fn_residential+parameters.fn_pts+suffix_out_shp, point_shapes, residential_pts, shapefile.POINT, utils.reducedFields)
	utils.toFile(parameters.fn_prefix+parameters.fn_activities+parameters.fn_pts+suffix_out_shp, point_shapes, activities_pts, shapefile.POINT, utils.reducedFields)
	####################################################################################
