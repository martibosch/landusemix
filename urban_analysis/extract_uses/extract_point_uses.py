...
def main(points_shapefile, suffix_out_shp):
    ...
    pts_activities_shops = utils.filter_columns('shop', sub_activities_shops)
    pts_activities_amenities = utils.filter_columns(
        'amenity', sub_activities_amenities)
    pts_activities_leisure = utils.filter_columns(
        'leisure', sub_activities_leisure)
    pts_activities_buildings = utils.filter_columns(
        'building', sub_activities_buildings)
    pts_activities_landuse = utils.filter_columns(
        'landuse', sub_activities_landuse)
    pts_residential_buildings = utils.filter_columns(
        'building', sub_residential_buildings)
    pts_residential_landuse = utils.filter_columns(
    ...
    ##########################################################################
    # Save to file
    utils.to_file(parameters.fn_prefix + parameters.fn_residential + parameters.fn_pts +
                  suffix_out_shp, point_shapes, residential_pts, shapefile.POINT, utils.reduced_fields)
    utils.to_file(parameters.fn_prefix + parameters.fn_activities + parameters.fn_pts +
                  suffix_out_shp, point_shapes, activities_pts, shapefile.POINT, utils.reduced_fields)
    ##########################################################################
