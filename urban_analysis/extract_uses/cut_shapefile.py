...
def merge_file(removeParts, to_merge_in_shp, out_shp_merged):
    ...
    call_ogr(['ogr2ogr', "-f", "ESRI Shapefile", out_shp_merged,
              to_merge_in_shp[0], "-lco", "ENCODING=UTF-8", "-overwrite"])
    for i in range(1, len(to_merge_in_shp)):
        call_ogr(['ogr2ogr', '-update', '-append', out_shp_merged,
                  to_merge_in_shp[i], "-lco", "ENCODING=UTF-8"])
...
def merge_final_categories(folder, numCuts, removeParts):
    ...
    # Assumes files with points are named "pts" and has the respective
    # category name
    toMergeActivities = []
    toMergeResidential = []
    ...
    merge_file(removeParts, toMergeActivities,
               parameters.fn_activities_final + ".shp")
    merge_file(removeParts, toMergeResidential,
               parameters.fn_residential_final + ".shp")


def merge_different_cuts(folder, numCuts, removeParts):
    ...
    for file in os.listdir(dirF):  # For all files within the folder to process
        if file.endswith("q0.shp"):
            toMerge = [folder + file.replace("q0", "q" + str(i))
                       for i in range(0, numCuts)]
            merge_file(removeParts, toMerge, folder + file.replace("q0", ""))

##########################################################################
##########################################################################


def get_touching_quadrants(PolygonQuadrants, Polygon):
    """ Get the different zones where a polygon touches/is contained
    """
    intersectsQuadrants = [i for i, quad in zip(
        range(0, len(PolygonQuadrants)), PolygonQuadrants) if quad.intersects(Polygon)]
...
def get_coords_arr_quadrants(bbox, numCuts):
    import math
    ...
    if (not(gridSize.is_integer())):
        print(
            'Number of cuts should be compatible with format [j*j] with j integer; Possibilities: 1,2,4,9,16,25,...')
        quit()
...
def get_polygon_quadrants(bbox, numCuts):
    """ Compute the bounding box coordinates of the different cuts
    Returns a polygon shape
    """
    Xs, Ys = get_coords_arr_quadrants(bbox, numCuts)
    ...
    for y_i in range(0, len(Ys) - 1):  # For all Y's values except the last one
        for x_i in range(0, len(Xs) - 1):  # For all X's values except the last one
            ...
            box = shapely.geometry.box(
                Xs[x_i], Ys[y_i], Xs[x_i + 1], Ys[y_i + 1])
            PolygonQuadrants.append(box)
    return PolygonQuadrants


def get_bbox_arr_quadrants(bbox, numCuts):
    """ Compute the bounding box coordinates of the different cuts
...
def clip_point_file(in_shp, out_shp):
    ...
    # Output files
    f1 = [out_shp + "_q" +
          str(i) + ".shp" for i in range(0, parameters.numCuts)]

    ####################################################################
    PolygonQuadrants = get_bbox_arr_quadrants(r.bbox, parameters.numCuts)

    # Output shapefile, bounding box quadrant
    for f, quad in zip(f1, PolygonQuadrants):
        call_ogr(["ogr2ogr", "-overwrite", "-f", "ESRI Shapefile", f,
                  in_shp, "-clipsrc", quad[0], quad[1], quad[2], quad[3]])

####################################


def clip_polygon_file(in_shp, out_shp):
    ...
    w_fq = [shapefile.Writer(r.shapeType)
    ...
    ####################################################################
    PolygonQuadrants = get_polygon_quadrants(r.bbox, parameters.numCuts)
    ...
    for poly in r.iterShapeRecords():
        ...
        quadrant = get_touching_quadrants(
            PolygonQuadrants, Polygon(geom.points))

        # If polygon lies within several quadrants, add it as a special
        # polygon:
        if (len(quadrant) > 1):
            w_several_quadrants._shapes.append(geom)
            w_several_quadrants.records.append(rec)

        # For each polygon, add it to one quadrant. It will be further
        # processed to be inferred
