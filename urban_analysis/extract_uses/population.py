...
def getContaining_LatitudeLongitude(GeoTransform, x0, y0):
    ...
    """
    # Top left and bottom right in terms of world coordinates
    # (latitude,longitude)
...
def getROI(GeoTransform, point_shapefile, polygon_shapefile=None, line_shapefile=None):
    ...
    bbox = utils.get_bounding_box(
        point_shapefile, polygon_shapefile, line_shapefile)

    # Get the pixel values (floating precision) of the extremes given
    # longitude/latitude
    Pix1 = coord2pixel(GeoTransform, bbox[0], bbox[1])
    Pix2 = coord2pixel(GeoTransform, bbox[2], bbox[3])

    # Get min and max values, since ReadAsAray will interprete as an image:
    # (0,0) at top left
    # Round DOWN the minimum value pixel X
    x1 = int(math.floor(min(Pix1[0], Pix2[0])))
    # Round UP the maximum value pixel X
    x2 = int(math.ceil(max(Pix1[0], Pix2[0])))
    # Round DOWN the minimum value pixel Y
    y1 = int(math.floor(min(Pix1[1], Pix2[1])))
    # Round UP the maximum value pixel Y
    y2 = int(math.ceil(max(Pix1[1], Pix2[1])))
    return x1, y1, x2, y2
############################################################################
...
def population_downscaling(population_count_file, residential_point_shapefile):
    ...
    # xoff/yoff are image left corner, a/e are pixel width/height and b/d is
    # rotation and is zero if image is north up.
    ...
    shape_residential_pts, df_residential_pts = utils.read_shp_dbf(
        residential_point_shapefile)
    ...
    s_residential_pts = pd.Series(
        [Point(shape.points[0]) for shape in shape_residential_pts])
    ##########################

    if (parameters.USE_verbose):
        start_time = time.time()

    # For the given raster, assign to the corresponding residential points its
    # population count
    for row in range(0, population_np.shape[0]):
        for col in range(0, population_np.shape[1]):
            ...
            # Get the latitude,longitude bounding box to identify points
            # contained within the box
            lat1, long1, lat2, long2 = getContaining_LatitudeLongitude(
                GeoTransform, x, y)
            # Create a geometry box for the containing coordinates
            Bbox = geo.box(lat1, long1, lat2, long2)

            # For all residential points, pick those which are contained in the
            # bounding box
            within_residential_pts = s_residential_pts[
                s_residential_pts.apply(lambda s: Bbox.contains(s))]

            # Assign pop_count distributed in an homogeneous way across the
            # residential points
            homogeneousPopCountDistribution = pop_count / \
                len(within_residential_pts)

            # Set the value
            df_residential_pts.loc[
                within_residential_pts.index, 'value'] = homogeneousPopCountDistribution

    if (parameters.USE_verbose):
        print("--- %s seconds ---" % (time.time() - start_time))

    # Save file
    utils.to_file(parameters.fn_prefix + parameters.fn_final_clasif + parameters.fn_residential,
                  shape_residential_pts, df_residential_pts, shapefile.POINT, utils.reduced_fields)
    ##########################################################################
