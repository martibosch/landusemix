...
def clip_files(point_shapefile, polygon_shapefile):
    ...
    cut_shapefile.clip_polygon_file(
        polygon_shapefile, parameters.fn_subregions + parameters.fn_poly)
    gc.collect()

    cut_shapefile.clip_point_file(
        point_shapefile, parameters.fn_subregions + parameters.fn_pts)
    gc.collect()
    ################################################################


def perform_extraction(population_count_file=None):
    ...
    for i in range(0, parameters.numCuts):
        ...
        # Get name files
        in_shp_pts = parameters.fn_subregions + \
            parameters.fn_pts + "_q" + str(i)
        in_shp_polys = parameters.fn_subregions + \
            parameters.fn_poly + "_q" + str(i)
        in_shp_polys_sq = parameters.fn_subregions + \
            parameters.fn_poly + "_sq"  # Several quadrants polygons
        out_suffix = "_q" + str(i)
        ...
        PolyToPoint_needed = extract_poly_uses.getNameSavedFiles(
            out_suffix) + infer_poly_uses.getNameSavedFiles(out_suffix)
        poly_to_pts.convert(PolyToPoint_needed)
        gc.collect()

    # Merge the different cuts into one file
    cut_shapefile.merge_different_cuts(
        parameters.fn_prefix, parameters.numCuts, parameters.deleteParts)

    # Merge the categories: Use only points
    if (parameters.USE_Merge_categories):
        cut_shapefile.merge_final_categories(
            parameters.fn_prefix, parameters.numCuts, parameters.deleteMergedParts)
        # Map to final activity categories
        utils.perform_key_category_mapping(parameters.fn_activities_final)

    ########
    if (population_count_file != None):
        # Process: Perform population down-scaling and distribute to each
        # residential point
        population.population_downscaling(
            population_count_file, parameters.fn_residential_final)
        gc.collect()
    ########
...
def process(point_shapefile, polygon_shapefile, population_count_file=None):
    ...
    if (os.path.isdir(parameters.fn_prefix)):
        print('Folder already exists. Assumption: Already processed, then ignoring...',
              parameters.fn_prefix)
        return
    ################################################################
    ...
    ################################################################
    ################################################################
    # Clip files in parts (reduced memory needs if map is too big, faster
    # polygon infer)
    clip_files(point_shapefile, polygon_shapefile)

    # Extract residential/activity uses. Alternatively, estimate the
    # population count for residential points
    perform_extraction(population_count_file)
    ...
    ################################################################
    if (parameters.USE_verbose):
        print("Complete processing: --- %s seconds ---" %
...
def main():
	...
	'''
    process(parameters.point_shapefile, parameters.polygon_shapefile,
            parameters.population_count_file)
