...
def get_name_saved_files(PolyToPoint_needed):
...
def convert_polys_to_points(in_shp_poly_file, out_shp_pts_file):
    ...
    if (parameters.USE_verbose):
        print('poly_to_pts. Dataset; Number of points',
              out_shp_pts_file, len(Points_atr))

    utils.to_file(out_shp_pts_file, Points_xy, df_polygon,
                  shapefile.POINT, utils.reduced_fields)

    if (parameters.deletePolyToPts):
        cut_shapefile.remove_files(in_shp_poly_file + ".shp")
...
def convert(PolyToPoint_needed):
    ...
    for i in PolyToPoint_needed:
        convert_polys_to_points(i, i.replace(
            parameters.fn_poly, parameters.fn_pts_from_poly))
    ##########################################################################
